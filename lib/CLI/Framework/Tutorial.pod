...
documentation in C<CLI::Framework::Application> and C<CLI::Framework::Command>,
which have more thorough coverage of some finer points.
...
C<CLI::Framework::Application> enumerates many advantages to using CLIF
...
databases, etc.) have been supplied, fail nicely if something is missing or
inconsistent, then do something that depends on the options, arguments, and
external resources.
...
people learning your tools have to have lengthy, verbal one-on-one code tours.
...
=head1 FROM SCRIPT TO CLIF IN A FEW EASY STEPS

Please see working code for this example included with the tests for the
C<CLI::Framework> distribution (F<examples/demo-simple.pl>).
...
strictly optional) that inherits from L<CLI::Framework::Application> and
define a method, C<command_map()>, that links command names with command
...
structure to be returned is exactly as explained in
...
advantages (see
L<DESIGN GOALS AND FEATURES|CLI::Framework::Application/DESIGN GOALS AND FEATURES>
in the L<CLI::Framework::Applicaton> for the long list):
...
See L<CONCEPTS AND DEFINITIONS|CLI::Framework::Application/CONCEPTS AND DEFINITIONS>
in the L<CLI::Framework::Application> POD.

=head1 UNDERSTANDING THE APPLICATION RUN SEQUENCE

See L<APPLICATION RUN SEQUENCE|CLI::Framework::Application/APPLICATION RUN SEQUENCE>
in the L<CLI::Framework::Application> POD.

B<Understanding this is important to building more complex apps>.  You need,
at the least, to understand how CLIF differentiates between application
options/args and those options/args that are meant for commands.
...
meant for the application as a whole or the specific command).
...
(notice that the final arguments apply to the subcommand.  In this form, the
only command that can receive arguments is the final subcommand).
...
Please see working code for this example included with the tests for the
C<CLI::Framework> distribution (F<examples/queue>).
...
Suppose we need to write a command-line app that provides an interface to a
...
it is for business logic -- little additional CLIF code is needed.

The example code shows how various commands can be managed by an Application
subclass.  The code is commented thouroughly to explain the various hooks that
are available for Application Class and Command Classes.

Each application can also be used in non-interactive mode:

    # ---- non-interactive mode (not supported by example code) ----
...
Basic CLIF apps don't require a lot of special thought -- the strategy
explained in L<FROM SCRIPT TO CLIF IN A FEW EASY STEPS> demonstrates how CLIF
differs from a "plain old script."

A more complicated command line application will benefit from a wider variety
of the features CLIF provides.  The extra features are easy to use, but the
additional complexity warrants some planning.
...
What commands and subcommands should be available?  What options and arguments
will they support?  What kind of validation should be done on the provided
command requests?  Which built-in commands will be used?  Will an interactive
mode be provided?  If so, will a custom menu be created?  Do any commands
need to directly access or modify the application itself or the other commands
(these will be metacommands)?
...
This section explains how CLIF could be used to support various common goals.
Even if your particular situation does not appear here, reading this section
will give you an understanding of how CLIF could be set up to support novel
cases.

=head2 How can I quickly create a very simple application?

CLIF applications require, at the minimum:

=over

=item *

An Application Class that inherits from L<CLI::Framework::Application>.  For
anything useful to happen, it should override the C<command_map()> hook and
...
L<CLI::Framework::Application|/cache>, where the object will be available to your
application and command objects.
...
You may also want to override L<read_cmd|CLI::Framework::Application/read_cmd>.
...
=item FIXME-TODO:finish writing troubleshooting section

* if user-defined command class is used in preference to builtin, check
perl -wc <user-defined-class> because user-defined classes that fail
compilation will fail silently

* clear up potential misunderstandings about loading subcommands from
files vs. inline
